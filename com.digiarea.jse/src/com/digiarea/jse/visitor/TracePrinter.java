...
public class TracePrinter implements VoidVisitor<SourcePrinter> {
	...
	/**
	 * Instantiates a new trace printer.
	 */
	public TracePrinter() {
		super();
	}
	...
	protected void printJavadoc(JavadocComment javadoc, SourcePrinter printer)
			throws Exception {
		if (javadoc != null && javadoc.getContent() != null) {
			printer.print(javadoc.getContent());
		}
	}
	...
	public <E extends Node> void visit(NodeList<E> n, SourcePrinter printer)
		...
		// FIXME dirty hack for hashCode - replace after ticket #238 wiil be
		// complied
		// throw new Exception("No literal presentation for this node");
		printer.print("[");
		if (n.getNodes() != null) {
			for (Iterator<? extends Node> i = n.getNodes().iterator(); i
					.hasNext();) {
				Node e = i.next();
				e.accept(this, printer);
				if (i.hasNext()) {
